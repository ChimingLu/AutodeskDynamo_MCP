    ...
    try:
        async with sse_client("http://127.0.0.1:8000/sse") as streams:
            async with ClientSession(streams[0], streams[1]) as session:
                ...
                # We expect a warning if the node is missing, that's fine for this test
                if "mcp_warning" in status_data:
                    print(f"Note: {status_data['mcp_warning']}")
                else:
                    print("Status: OK")

                # Draw random line v16
                ...
                print(f"Drawing random line v16 from ({x1}, {y1}, {z1}) to ({x2}, {y2}, {z2})...")
                
                code_content = f"p_start_16 = Point.ByCoordinates({x1}, {y1}, {z1});\np_end_16 = Point.ByCoordinates({x2}, {y2}, {z2});\nline_16 = Line.ByStartPointEndPoint(p_start_16, p_end_16);"
