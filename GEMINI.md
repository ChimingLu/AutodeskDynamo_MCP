...
- **偵測與授權 (模式 B)**：
    - **定義**：WebSocket 客戶端已連線，但伺服器未收到「Start 節點」宣告。
    - **特徵**：執行指令回報「Authorization Required」。
    - **處理邏輯**：提示使用者放下 `StartMCPServer` 節點。一旦放置，`WebSocketClient` 會自動觸發事件回報給伺服器，AI 隨即恢復執行能力。
- **幽靈連線徹底修復**：
    - WebSocket 方案透過持久性 TCP 連線解決了幽靈連線問題。若連線中斷，伺服器會立即感知並清除 Session。
- **強制回報**：每次連線建立或環境變動後，AI 會收到：
    - `Workspace FileName`: 確認當前開啟的檔案。
    - `hasStartNode`: 確認是否具備操作權限。
...
### 🎯 核心教訓 #1：回歸 Code Block 至上策略 (Track A Supremacy)

> **最後更新**: 2026-01-16  
> **重大變更**: 由於原生節點自動擴展 (Track B) 目前存在架構不穩定問題（連線失敗、殭屍節點），**已強制停用 Track B 擴展邏輯**。

**核心行為規範**：
- ❌ **嚴禁使用** `params` 欄位的原生節點格式。
- ✅ **強制使用** `"name": "Number"` 搭配 `value` 欄位的 DesignScript 格式。
- ✅ **幾何創建 100% 使用 Code Block**：所有 Point、Line、Cuboid 必須內嵌在單一 Code Block 中。

**原因**：Code Block 方法不依賴複雜的連線機制，具備 100% 的成功率與系統韌性。

---

#### 🛤️ 穩定路徑：Code Block 內嵌模式 (Track A)
...
**⚠️ 軌道 B 維修中警告**：
當前伺服器設定 `ENABLE_AUTO_EXPANSION = False`。若 AI 嘗試發送參數化 JSON，輸入將會遺失。

---

### 🚫 核心教訓 #2：軌道 A 的關鍵原則 (避免分散式節點)

**過時反模式** (軌道 B 出現前的錯誤做法):  
試圖創建多個 `Number` 節點分別存放 X、Y、Z 值，再透過 `connectors` 連接到原生節點。
...
```json
{"name": "Number", "value": "Line.ByStartPointEndPoint(Point.ByCoordinates(0,0,0), Point.ByCoordinates(100,100,100));"}
```

**軌道 B 的突破**：
透過 `server.py` 的自動擴展機制 + `GraphHandler.cs` 的 ID 映射，已解決連線同步問題。但僅適用於**獨立節點**，不適用於**嵌套幾何**。
